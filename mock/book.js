...
export function removeByID(req, res, u){
    const result = {
       ...
       data : {
         _id  : res.req.body.id,
         name : `(删)剧本类型${Math.floor(Math.random() * 200)}`
       }
    }
    if (res && res.json) {
      res.json(result);
...
export function updateByID(req, res, u){
  const result = {
     ...
     data : {
       _id  : res.req.body.id,
       name : `(改)剧本类型${Math.floor(Math.random() * 200)}`
     }
  }
  if (res && res.json) {
    res.json(result);
...
export function create(req, res, u){
  const result = {
    ...
    msg :'未知错误',     // 测试，只有在success为false情况下显示
    data : {
      _id  : `BOOK${Math.floor(Math.random() * 1000)}`,
      name : `(增)剧本类型${Math.floor(Math.random() * 200)}`
    }
  }
  if (res && res.json) {
    res.json(result);
